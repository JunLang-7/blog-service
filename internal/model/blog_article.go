...
func (a *BlogArticle) ListByTagID(db *gorm.DB, tagID uint32, pageOffset, pageSize int) ([]*ArticleRow, error) {
	...
	rows, err := db.Select(fields).Table(new(BlogTagArticle).TableName()+"AS at").
...
func (a *BlogArticle) CountByTagID(db *gorm.DB, tagID uint32) (int, error) {
	var count int64
	err := db.Table(new(BlogTagArticle).TableName()+"AS at").
		Joins("LEFT JOIN `"+new(BlogArticle).TableName()+"` AS ar ON at.article_id = ar.id").
		Where("at.`tag_id` = ? AND ar.state = ? AND ar.is_del = ?", tagID, a.State, 0).
		Count(&count).Error
	if err != nil {
